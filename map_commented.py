...
def init():
    ...
    global goal_x # x-coordinate of the goal (where the find has to go, that is the airport or the downtown)
    global goal_y # y-coordinate of the goal (where the find has to go, that is the airport or the downtown)
    sand = np.zeros((longueur,largeur)) # initializing the sand array with only zeros
    goal_x = 20 # the goal to reach is at the upper left of the map (the x-coordinate is 20 and not 0 because the find gets bad reward if it touches the wall)
...
# Creating the find class (to understand "NumericProperty" and "ReferenceListProperty", see kivy tutorials: https://kivy.org/docs/tutorials/pong.html)

class Find(Widget):

    angle = NumericProperty(0) # initializing the angle of the find (angle between the x-axis of the map and the axis of the find)
    rotation = NumericProperty(0) # initializing the last rotation of the find (after playing the action, the find does a rotation of 0, 20 or -20 degrees)
    ...
    def move(self, rotation):
        self.pos = Vector(*self.velocity) + self.pos # updating the position of the find according to its last position and velocity
        self.rotation = rotation # getting the rotation of the find
        ...
        if self.sensor1_x > longueur-10 or self.sensor1_x<10 or self.sensor1_y>largeur-10 or self.sensor1_y<10: # if sensor 1 is out of the map (the find is facing one edge of the map)
            self.signal1 = 1. # sensor 1 detects full sand
        if self.sensor2_x > longueur-10 or self.sensor2_x<10 or self.sensor2_y>largeur-10 or self.sensor2_y<10: # if sensor 2 is out of the map (the find is facing one edge of the map)
            self.signal2 = 1. # sensor 2 detects full sand
        if self.sensor3_x > longueur-10 or self.sensor3_x<10 or self.sensor3_y>largeur-10 or self.sensor3_y<10: # if sensor 3 is out of the map (the find is facing one edge of the map)
...
class Game(Widget):
    ...
    find = ObjectProperty(None) # getting the find object from our kivy file
    ...
    def serve_find(self): # starting the find when we launch the application
        self.find.center = self.center # the find will start at the center of the map
        self.find.velocity = Vector(6, 0) # the find will start to go horizontally to the right with a speed of 6
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        global brain # specifying the global variables (the brain of the find, that is our AI)
        ...
        global last_distance # specifying the global variables (the last distance from the find to the goal)
        ...
        xx = goal_x - self.find.x # difference of x-coordinates between the goal and the find
        yy = goal_y - self.find.y # difference of y-coordinates between the goal and the find
        orientation = Vector(*self.find.velocity).angle((xx,yy))/180. # direction of the find with respect to the goal (if the find is heading perfectly towards the goal, then orientation = 0)
        last_signal = [self.find.signal1, self.find.signal2, self.find.signal3, orientation, -orientation] # our input state vector, composed of the three signals received by the three sensors, plus the orientation and -orientation
        ...
        self.find.move(rotation) # moving the find according to this last rotation angle
        distance = np.sqrt((self.find.x - goal_x)**2 + (self.find.y - goal_y)**2) # getting the new distance between the find and the goal right after the find moved
        self.ball1.pos = self.find.sensor1 # updating the position of the first sensor (ball1) right after the find moved
        self.ball2.pos = self.find.sensor2 # updating the position of the second sensor (ball2) right after the find moved
        self.ball3.pos = self.find.sensor3 # updating the position of the third sensor (ball3) right after the find moved

        if sand[int(self.find.x),int(self.find.y)] > 0: # if the find is on the sand
            self.find.velocity = Vector(1, 0).rotate(self.find.angle) # it is slowed down (speed = 1)
            last_reward = -1 # and reward = -1
        else: # otherwise
            self.find.velocity = Vector(6, 0).rotate(self.find.angle) # it goes to a normal speed (speed = 6)
        ...
        if self.find.x < 10: # if the find is in the left edge of the frame
            self.find.x = 10 # it is not slowed down
            last_reward = -1 # but it gets bad reward -1
        if self.find.x > self.width-10: # if the find is in the right edge of the frame
            self.find.x = self.width-10 # it is not slowed down
            last_reward = -1 # but it gets bad reward -1
        if self.find.y < 10: # if the find is in the bottom edge of the frame
            self.find.y = 10 # it is not slowed down
            last_reward = -1 # but it gets bad reward -1
        if self.find.y > self.height-10: # if the find is in the upper edge of the frame
            self.find.y = self.height-10 # it is not slowed down
            last_reward = -1 # but it gets bad reward -1

        if distance < 100: # when the find reaches its goal
        ...
        # Updating the last distance from the find to the goal
